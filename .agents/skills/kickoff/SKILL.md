...
1. When `/启动项目` is invoked, spawn the planning agent
...
1. **File path**: A path to an inbox note (e.g., `/启动项目 00_收件箱/MyIdea.md`) - read the file contents
2. **Inline text**: A short description of a project idea (e.g., `/启动项目 Build a habit tracker app`)
...
When the user invokes `/启动项目` with their idea, immediately spawn a planning agent using the Task tool:
